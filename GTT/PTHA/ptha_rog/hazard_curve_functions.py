...
def plot_hazard_curve(h_ev,p_ev):
    ...
    text(h_ev[-1]*0.6, 2e-1, 'P(%4.2f) = %9.7f' % (h,p))
    h = 1.3
    p = p_interp(h,h_ev,p_ev)
    text(h_ev[-1]*0.6, 1e-1, 'P(%4.2f) = %9.7f' % (h,p))
    h = 1.8
    p = p_interp(h,h_ev,p_ev)
    text(h_ev[-1]*0.6, 5e-2, 'P(%4.2f) = %9.7f' % (h,p))
    subplot(122)
    plot(h_ev,p_ev)
...
def makefig(h2,lambda2):
    ...
    ax.text(0.5, 0.5e-1, 'lambda2 = %5.4f' % lambda2)
    ax = fig.axes[1]
